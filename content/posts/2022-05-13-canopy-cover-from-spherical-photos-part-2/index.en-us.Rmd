...
leaflet() %>% 
  addProviderTiles("Esri.WorldImagery") %>%
  addRasterImage(interp_TPS, colors = pal, layerId = "raster") %>% 
  leaflet::addLegend(
    position = "topright",
    pal = pal,
...
by playing with the transparency slider you can see how closely the gap fraction (a good measure of canopy cover) coincides with the eucalyptus trees. That looks like a pretty good correlation just by looking at it, in the next part we will look at how to test that correlation against other variables and methods.
